...
export const handleTimerStart = async (io: any, socket: any, { sessionId, duration }: { sessionId: string; duration?: number }) => {
  try {
...
export const handleTimerStop = (io: any, socket: any, { sessionId }: { sessionId: string }) => {
  try {
...
export const handleTimerReset = (io: any, socket: any, { sessionId, duration }: { sessionId: string; duration?: number }) => {
  try {
    if (socket.data.timerInterval) {
      clearInterval(socket.data.timerInterval);
    }
